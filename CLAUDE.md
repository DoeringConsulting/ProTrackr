...
1. `git commit` auf `main` → `pre-commit` Hook führt Tests aus (`server/taxEnginePl.test.ts`, `server/uiValidationReportsDashboard.test.ts`, `server/revenueForecast.test.ts`, `server/monthlyFinancials.test.ts`, `server/expensePeriodAttribution.test.ts`).
